...
def _eps_greedy(
    Q: np.ndarray, s: int, eps: float, n_actions: int, rng: np.random.Generator
) -> int:
    if rng.random() < eps:
...
def _make_grid_fig(sarsa_path: list, ql_path: list, wind: list) -> go.Figure:
    ...
    # Labels: S and G
    fig.add_annotation(
        x=0, y=3, text="<b>S</b>", showarrow=False, font=dict(size=15, color="#1A5276")
    )
    fig.add_annotation(
        x=GC,
        y=GR,
    ...
    for c, w in enumerate(wind):
        ...
        fig.add_annotation(
            x=c, y=7.4, text=label, showarrow=False, font=dict(size=13, color="#AAAAAA")
        )

    fig.update_layout(
        title="Windy GridWorld — Learned Paths (greedy policy)",
        xaxis=dict(
            tickvals=list(range(COLS)), title="Column", showgrid=False, zeroline=False
        ),
        yaxis=dict(
            autorange="reversed",
...
""")
    ...
    if st.button("Run Simulation", type="primary"):
        ...
        if not sarsa_reached or not ql_reached:
            who = []
            if not sarsa_reached:
                who.append("SARSA")
            if not ql_reached:
                who.append("Q-Learning")
            st.warning(
        ...
        # Grid with paths
        fig_grid = _make_grid_fig(
            result["sarsa_path"], result["ql_path"], result["wind"]
        )
