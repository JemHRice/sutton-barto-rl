...
""")
    ...
    if st.button("Run Simulation", type="primary"):
        ...
        avg_rewards, final_Q = run_epsilon_greedy(
        ...
        fig_q.add_trace(
            go.Bar(
                x=[arm_labels[i] for i in sorted_idx],
                y=final_Q[sorted_idx],
                name="Estimated Q (last episode)",
                marker_color="#636EFA",
                opacity=0.7,
            )
        )
        ...
        with col_a:
            st.metric(
                "Best arm", f"Arm {best_arm + 1}", f"q* = {means_arr[best_arm]:.3f}"
            )
        with col_b:
            st.metric(
                "Final avg reward (last 100 steps)", f"{avg_rewards[-100:].mean():.3f}"
            )
        with col_c:
